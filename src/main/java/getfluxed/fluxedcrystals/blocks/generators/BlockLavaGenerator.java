...
public class BlockLavaGenerator extends BlockContainer {
    ...
    public static void setState(boolean active, World worldIn, BlockPos pos) {
        ...
        worldIn.setBlockState(pos, FCBlocks.lavaGenerator.getDefaultState().withProperty(FACING, iblockstate.getValue(FACING)).withProperty(isActive, active), 3);
        worldIn.setBlockState(pos, FCBlocks.lavaGenerator.getDefaultState().withProperty(FACING, iblockstate.getValue(FACING)).withProperty(isActive, active), 3);

        if (tileentity != null) {
            tileentity.validate();
            worldIn.setTileEntity(pos, tileentity);
            PacketHandler.INSTANCE.sendToAllAround(new MessageGenerator((TileEntityLavaGenerator) tileentity), new NetworkRegistry.TargetPoint(worldIn.provider.getDimension(), (double) tileentity.getPos().getX(), (double) tileentity.getPos().getY(), (double) tileentity.getPos().getZ(), 128d));
        }
    }

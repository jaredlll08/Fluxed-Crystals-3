...
public class BlockCoalGenerator extends BlockContainer {
    ...
    public static void setState(boolean active, World worldIn, BlockPos pos) {
        ...
        worldIn.setBlockState(pos, FCBlocks.basicCoalGenerator.getDefaultState().withProperty(FACING, iblockstate.getValue(FACING)).withProperty(isActive, active), 3);
        worldIn.setBlockState(pos, FCBlocks.basicCoalGenerator.getDefaultState().withProperty(FACING, iblockstate.getValue(FACING)).withProperty(isActive, active), 3);
    ...
    public boolean onBlockActivated(World worldIn, BlockPos pos, IBlockState state, EntityPlayer playerIn, EnumHand hand, ItemStack heldItem, EnumFacing side, float hitX, float hitY, float hitZ) {
        if (!worldIn.isRemote) {
            playerIn.openGui(FluxedCrystals.instance, 0, worldIn, pos.getX(), pos.getY(), pos.getZ());
        }
        return true;
    }

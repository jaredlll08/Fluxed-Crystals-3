package com.blamejared.fluxedcrystals.util.models;
...
public class OBJColourModel implements IRetexturableModel, IModelCustomData {
		...
		public OBJColourModel parse() throws IOException {
			...
			for(; ; ) {
				...
				try {
					...
					{
						float[] coords = parseFloats(splitData);
						OBJColourModel.TextureCoordinate texCoord = new OBJColourModel.TextureCoordinate(coords[0], coords.length >= 2 ? coords[1] : 0.0F, coords.length >= 3 ? coords[2] : 0.0F);
		...
		public OBJColourModel.Face bake(TRSRTransformation transform) {
			...
			for(int i = 0; i < verts.length; i++) {
				...
				if(v.hasTextureCoordinate())
					...
					v.setTextureCoordinate(OBJColourModel.TextureCoordinate.getDefaultUVs()[i]);
	...
	public static class OBJState implements IModelState {
		...
		public IModelState getParent(IModelState parent) {
			if(parent == null)
				return null;
			else if(parent instanceof OBJColourModel.OBJState)
				return ((OBJColourModel.OBJState) parent).parent;
			return parent;
		}
		...
		public boolean equals(Object obj) {
			...
			return com.google.common.base.Objects.equal(visibilityMap, other.visibilityMap) && com.google.common.base.Objects.equal(parent, other.parent) && operation == other.operation;
		}
		...
		private final void putVertexData(UnpackedBakedQuad.Builder builder, OBJColourModel.Vertex v, OBJColourModel.Normal faceNormal, OBJColourModel.TextureCoordinate defUV, TextureAtlasSprite sprite) {
			for(int e = 0; e < format.getElementCount(); e++) {
				switch(format.getElement(e).getUsage()) {
					...
					case UV:
						if(!v.hasTextureCoordinate())
							builder.put(e, sprite.getInterpolatedU(defUV.u * 16), sprite.getInterpolatedV((model.customData.flipV ? 1 - defUV.v : defUV.v) * 16), 0, 1);
						else
							builder.put(e, sprite.getInterpolatedU(v.getTextureCoordinate().u * 16), sprite.getInterpolatedV(v.getTextureCoordinate().v * 16), 0, 1);
						break;
					case NORMAL:
						if(!v.hasNormal())
